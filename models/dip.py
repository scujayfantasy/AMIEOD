...
from models.filters import DefogFilter,ImprovedWhiteBalanceFilter,GammaFilter,\
...
class Dip_Cnn(nn.Module):
    ...
    def forward(self, x):
        x = nn.functional.interpolate(x.clone(), size=(256,256), mode="bilinear", align_corners=False)
        out = self.cnnnet(x)
