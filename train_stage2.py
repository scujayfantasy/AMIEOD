...
import val_sci_offline_online_dip_stage2 as validate  # for end-of-epoch mAP
from models.experimental import attempt_load
from models.yolo_sci_dip_selector_init_256 import Model
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for name, param in model.enhance.named_parameters():
        param.requires_grad = False
    for name, param in model.dip_model.named_parameters():
        param.requires_grad = False
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                with torch.no_grad():
                    ...
                    pred_ori = model(imgs)[1]
                    pred_dip = model(imgs_dip)[1]
                    pred_sci_offline = model(imgs_sci_offline)[1]
                    pred_sci_online = model(imgs_sci_online)[1]
                ...
                pre_en_type = model.selector(imgs)   #输出四个增强的
                loss_label = torch.tensor([loss_ori, loss_dip, loss_sci_offline, loss_sci_online])
                loss_label  = torch.argmin(loss_label).unsqueeze(0).to(device)
                # print(loss_label)
                pred_type = torch.argmax(pre_en_type)
                # print("true:",loss_label, "pre:", pred_type)

                selector_loss = nn.CrossEntropyLoss()(pre_en_type, loss_label)   #使模型输出 偏向于损失更小的，  模型认为哪个增强方法的增强后的图像 检测损失最小
                # print(selector_loss)

                if loss_label == 0:
                    loss, loss_items = loss_ori, loss_items_ori
                elif loss_label == 1:
                    loss, loss_items = loss_dip, loss_items_dip
                elif loss_label == 2:
                    loss, loss_items = loss_sci_offline, loss_items_sci_off
                elif loss_label == 3:
                    loss, loss_items = loss_sci_online, loss_items_sci_on



                    # loss = selector_loss
                # loss = (loss_ori + loss_dip + loss_sci_online + loss_sci_offline)
                # loss /= 4
            ...
            # Backward
            scaler.scale(selector_loss).backward()
        ...
        # 打开一个文本文件，准备写入每一轮的损失
        selector_loss_path = os.path.join(save_dir, "selector_loss.txt")
        with open(selector_loss_path, "a") as f:
            # 在文件中写入标题（可选）
            # print("true:", loss_label, "pre:", pred_type)
            f.write(f"Epoch{epoch}_Selector_Loss: {selector_loss}    ")
            f.write(f"Epoch{epoch}_pred_true_en: true_{loss_label.item()} pre_{pred_type.item()}\n")
