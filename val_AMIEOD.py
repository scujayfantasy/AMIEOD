...
def get_new_file_name(path, base_name, extension):
    """
    检查指定路径下是否存在同名文件，如果存在则自动编号递增。

    Args:
        path (str): 文件夹路径。
        base_name (str): 文件的基础名称（不含编号和扩展名）。
        extension (str): 文件扩展名（例如 '.txt', '.jpg'）。
...
def run(
    ...
    enhance_pretrain.half() if half else enhance_pretrain.float()  #

    # import thop
    # input_image_size = opt.imgsz
    # input_image = torch.randn(1, 3, input_image_size, input_image_size).to(device)
    # flops, params = thop.profile(model.model, inputs=(input_image,), verbose=False)
    ...
    save_val_file = get_new_file_name(path=save_dir, base_name="selector_val_en_type", extension=".txt")
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[1]:
            ...
            if training:
                ...
                pre_en_type = model.selector(im)   #模型预测哪个增强方式带来的检测损失最小
                en_type = torch.argmax(pre_en_type).item()#unsqueeze(0).to(device)
            # en_type = pre_en_type.argsort(0, descending=True)
            # en_type.append(pre_en_type.argsort(1, descending=True)[:, :5])

                if en_type == 1:
                    dip_param = model.dip_model(im)
                    imgs_en = Dip_filters(dip_param, cfg_dip, im).half() if half else Dip_filters(dip_param, cfg_dip, im).float()
                elif en_type == 2:
                    imgs_en = enhance_pretrain(im)[1].half() if half else enhance_pretrain(im)[1].float()
                elif en_type == 3:
                    imgs_en = model.enhance(im)[1].half() if half else model.enhance(im)[1].float()
                else: #en_type == 0
                    imgs_en = im.half() if half else im.float()
            else:
                pre_en_type = model.model.selector(im)  # 模型预测哪个增强方式带来的检测损失最小
                en_type = torch.argmax(pre_en_type).item()  # unsqueeze(0).to(device)
                # en_type = pre_en_type.argsort(0, descending=True)
                # en_type.append(pre_en_type.argsort(1, descending=True)[:, :5])

                if en_type == 1:
                    dip_param = model.model.dip_model(im)
                    imgs_en = Dip_filters(dip_param, cfg_dip, im).half() if half else Dip_filters(dip_param, cfg_dip, im).float()
                elif en_type == 2:
                    imgs_en = enhance_pretrain(im)[1].half() if half else enhance_pretrain(im)[1].float()
                elif en_type == 3:
                    imgs_en = model.model.enhance(im)[1].half() if half else model.model.enhance(im)[1].float()
                else:  # en_type == 0
                    imgs_en = im.half() if half else im.float()
            ...
            preds, train_out = model(imgs_en) if compute_loss else (model(imgs_en, augment=augment), None)
        ...
        # 打开一个文本文件，准备写入每一轮的损失
        selector_loss_path = os.path.join(save_dir,save_val_file)
        with open(selector_loss_path, "a") as f:
            f.write(f"Val_pred_en: pre_{en_type}\n")
