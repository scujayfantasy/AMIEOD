...
from models.yolo_sci_dip_noselector import Model
from models.dip import cfg as cfg_dip
from models.dip import Dip_filters
...
def pred_singleimg(pred,device):
    ...
    # 对每个 tensor 按照 batch 分割
    for tensor in pred:
        # 按batch维度分割，每个split的大小为1
        split_tensor = torch.split(tensor, 1, dim=0) # 在batch维度上按1拆分
        split_preds.append(split_tensor)

    # 生成最终的嵌套列表，其中每个元素是一个包含三个tensor的子列表
    final_result = []
    for i in range(batch):
        final_result.append([split_preds[0][i], split_preds[1][i], split_preds[2][i]])
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # for name, param in model.enhance.named_parameters():
    #     param.requires_grad = False

    from models.enhance import Finetunemodel as enhance_pretrain
    enhance_pretrain = enhance_pretrain("./models/enhance/weights/medium.pt")
    enhance_pretrain = enhance_pretrain.cuda()
    enhance_pretrain.eval()
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        # enhance_img = cv2.imread("/home/bobo/H/hxc/low-light/yolov3/models/enhance/data/medium/00001.png")
        # enhance_img = enhance_img[:, :, ::-1].copy()  #BGR2RGB
        #
        # enhance_img = torch.tensor(enhance_img).to(torch.float32) / 255
        # enhance_img = enhance_img.permute(2, 0, 1).unsqueeze(0)
        # enhance_img = enhance_img.cuda()
        #
        # img_enhance = enhance_pretrain(enhance_img)[1]  #
        #
        # image_numpy = img_enhance[0].cpu().float().detach().numpy()
        # image_numpy = (np.transpose(image_numpy, (1, 2, 0)))
        # im = Image.fromarray(np.clip(image_numpy * 255.0, 0, 255.0).astype('uint8'))
        #
        # im.save(f"./saveimg/epoch_{epoch}.png")
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                dip_param = model.dip_model(imgs)
                imgs_dip = Dip_filters(dip_param, cfg_dip, imgs)
                imgs_sci_online = model.enhance(imgs)[1]
                imgs_sci_offline = enhance_pretrain(imgs)[1]

                pred_ori = model(imgs)
                pred_dip = model(imgs_dip)
                pred_sci_offline = model(imgs_sci_offline)
                pred_sci_online = model(imgs_sci_online)

                # pred_sin_img = pred_singleimg(pred_ori,device)

                # for batch in range(len(pred_sin_img)):
                #
                #     loss_ori, loss_items_ori = compute_loss(pred_sin_img[batch], targets[batch].unsqueeze(0).to(device))
                #     loss_dip, loss_items_dip = compute_loss(pred_dip, targets.to(device))
                #     oss_sci_offline, loss_items_sci_off = compute_loss(pred_sci_offline, targets.to(device))
                #     loss_sci_offline, loss_items_sci_off = compute_loss(pred_sci_online, targets.to(device))
                #     loss_label = torch.tensor([loss_ori, loss_dip, loss_sci_offline, loss_sci_online])
                #     pre_en_type = model.selector(imgs)  #
                #     selector_loss = nn.CrossEntropyLoss()(pre_en_type, best_en_type)

                loss_ori, loss_items_ori = compute_loss(pred_ori, targets.to(device))
                loss_dip, loss_items_dip = compute_loss(pred_dip, targets.to(device))
                loss_sci_offline, loss_items_sci_off = compute_loss(pred_sci_offline, targets.to(device))
                loss_sci_online, loss_items_sci_on = compute_loss(pred_sci_online, targets.to(device))

                loss_label = torch.tensor([loss_ori, loss_dip, loss_sci_offline, loss_sci_online])
                loss_index = torch.argmin(loss_label)

                if loss_index == 0:
                    # loss_reg_dip        = nn.L1Loss()(imgs_dip, imgs)
                    # loss_reg_sci_online = nn.L1Loss()(imgs_sci_online, imgs)
                    loss_reg = (nn.L1Loss()(imgs_dip, imgs) +  nn.L1Loss()(imgs_sci_online, imgs)) / 2
                elif loss_index == 1:
                    loss_reg = nn.L1Loss()(imgs_sci_online, imgs_dip.detach())
                    # loss_reg_sci_online = nn.L1Loss()(imgs_sci_online, imgs_dip.detach())
                elif loss_index == 2:
                    loss_reg =  (nn.L1Loss()(imgs_dip, imgs_sci_offline.detach()) + nn.L1Loss()(imgs_sci_online, imgs_sci_offline.detach())) / 2
                    # loss_reg_dip        = nn.L1Loss()(imgs_dip, imgs_sci_offline.detach())
                    # loss_reg_sci_online = nn.L1Loss()(imgs_sci_online, imgs_sci_offline.detach())
                elif loss_index == 3:
                    loss_reg = nn.L1Loss()(imgs_dip, imgs_sci_online.detach())
                else:
                    loss_reg = None
                    # loss_reg_sci_online = nn.L1Loss()(imgs_dip, imgs_sci_online.detach())



                loss_det = (loss_ori + loss_dip + loss_sci_online + loss_sci_offline) / 4
                # loss_det /= 4
                loss = (1-opt.alpha) * loss_det + opt.alpha * loss_reg

                loss_items = loss_items_ori + loss_items_dip + loss_items_sci_on + loss_items_sci_off
                loss_items /= 4
...
def parse_opt(known=False):
    ...
    parser.add_argument("--alpha", type=float, default=0.8, help="Label smoothing epsilon")
